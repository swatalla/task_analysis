function spm_12_batch_preprocessing()
%-----------------------------------------------------------------------
...
for k = 1:size(subjs,1)
    ...
    % get structural file
    b.struct = ['Y:\Research_Data\KL2_Subject_Data\' subjs(k,1:4) '\' subjs(k,:) '\Structural\T1*.nii'];
    % for each run, get niftis
    for r = 1:4
        clear runfld d;
        runfld = ['Y:\Research_Data\KL2_Subject_Data\' subjs(k,1:4) '\' subjs(k,1:5) '0' subjs(k,6:end) '\Functional\' subjs(k,1:5) '0' subjs(k,6:end) '_run_' num2str(r)];
        cd(runfld); 
        d = dir('run*.nii');
